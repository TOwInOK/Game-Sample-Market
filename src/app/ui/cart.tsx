...
import { useGetPrice, useRemoveFromCart } from "@/app/api/cart";
  ...
  return (
    <div
      className={clsx(
        ...
        {
          " border-2 rounded-lg": paths.some((path) =>
    ...
    >
      <>
        <Link
          href="/cart"
          key="cart"
          className="fill-black stroke-black size-10 transition duration-300 ease-in-out transform hover:scale-110 hover:underline dark:fill-white"
        >
      ...
      >
        ...
        <button
          ...
          className="p-2.5 rounded-lg border-2  flex-col justify-center items-center gap-2.5 inline-flex"
        >
          {/* Button */}
          <div className=" text-3xl font-normal font-itim">pay all</div>
        </button>
      </div>
    </div>
  );
